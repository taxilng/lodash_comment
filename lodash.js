...
; (function () {
  ...
  var runInContext = (function runInContext (context) {
     ...
     * 我还是喜欢用 Object.prototype.toString.call() 方法
     */
    function baseGetTag (value) {
      if (value == null) {
        // 用这个判断，是为了效率更高吗？为什么不用 objectToString()
        // undefinded 的类型 和 null类型  undefinedTag = '[object Undefined]',
        return value === undefined ? undefinedTag : nullTag;
      }
      // symToStringTag = Symbol ? Symbol.toStringTag : undefined; 
      // 如果value为symbol类型会 返回true
      //下面都是返回 "[object Symbol]" 这样的
    ...
    function baseSlice (array, start, end) {
      var index = -1,
        length = array.length;
      // 当起始位置为负数，则加上length开始算起始位置，最小还是0
      if (start < 0) {
        start = -start > length ? 0 : (length + start);
      }
      // 限制结束位置 最大为length
      ...
      //如果start > end 则截取长度为0
      /** 无符号右移，首位会被0填充，然后往右移动。
        *作用就是非数字会转成0，然后还可以取整。
        *正数不变，如果是负数，返回负数 + 2的32次方
      **/
      // length现在变成了 截取长度了
      ...
      while (++index < length) {
        //第一次执行 index 已经为0了，那么result[0] = array[start]
    ...
    function isIterateeCall (value, index, object) {
      ...
      if (type == 'number'
        ...
        // 实际上就是 object[index] === value 就 return true 其他都是false
    ...
    function chunk (array, size, guard) {
      //isIterateeCall 就是判断 array === guard[size] 目前感觉意义不明
      if ((guard ? isIterateeCall(array, size, guard) : size === undefined)) {
        size = 1;
      } else { 
        // nativeMax = Math.max,
        // 先对 size取整 然后 与 0 取 最大值
        size = nativeMax(toInteger(size), 0);
      }
      // 获取数组的长度
      var length = array == null ? 0 : array.length;
      if (!length || size < 1) {
        // 返回空数组的情况， 数组长度为0，切割数组长度小于1
        return [];
      }
      var index = 0,
        resIndex = 0,
        // 解析 nativeCeil = Math.ceil, 长度/size 向上取整
        // 例如 数组长度为5，size为2  那么就是 Math.ceil(5/2) = 3
        result = Array(nativeCeil(length / size));

      while (index < length) {
        // baseSlice可以暂时想成slice，index的起始位置为0，结束位置为 index+size
        // 第二次循环 index起始位置为index+size ，结束位置为 index+ 2*size
        // 直到 index >= length 则跳出循环，返回 result
    ...
    function isLength (value) {
      // 必须是数字类型，必须大于等于0；必须是整数；最大值小于 最大安全数值
